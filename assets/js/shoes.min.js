...
// filter shoes & run baguetteBox
var allCheckboxes = document.querySelectorAll('input[type=checkbox]'), allShoes = Array.from(document.querySelectorAll('.shoe')), checked = {}; Array.prototype.forEach.call(allCheckboxes, function (a) { a.addEventListener('change', toggleCheckbox) }), getChecked('size'), getChecked('heel'), getChecked('color'), setVisibility(), baguetteBox.run('.baguette'); function toggleCheckbox(a) { getChecked(a.target.name), setVisibility() } function getChecked(a) { checked[a] = Array.from(document.querySelectorAll('input[name=' + a + ']:checked')).map(function (b) { return b.value }) } function setVisibility() { allShoes.map(function (a) { var b = !checked.size.length || intersection(Array.from(a.classList), checked.size).length, c = !checked.heel.length || intersection(Array.from(a.classList), checked.heel).length, d = !checked.color.length || intersection(Array.from(a.classList), checked.color).length; a.style.display = b && c && d ? 'block' : 'none' }) }
